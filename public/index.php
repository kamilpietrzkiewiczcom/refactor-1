...
try {
    ...
    $definitions = \App\Di\Definitions::getDefinitions($request);
    $builder->addDefinitions($definitions);
    $container = $builder->build();

    # call action controller
    # here we can build all routing that we would need
    # for simplicity I call here single controller
    $action = $container->get(\App\Adapter\Controller\ActionController::class);
    $class = new ReflectionClass(get_class($action));
    $invokeMetadata = $class->getMethod('__invoke');
    $invokeParams = $invokeMetadata->getParameters();
    $invokeParamType = $invokeParams[0]->getType();
    $factory = $invokeParamType->getName() . "Factory";
    $factory = new $factory();
